...
%token DOUBLEQUOTE EXPONENTIAL COMMA LIST DIVIDEEQUAL MINUSEQUAL MULEQUAL LESSTHAN LESSTHANEQUAL 
%token GREATERTHAN GREATERTHANEQUAL DOUBLEEQUAL NOTEQUAL GLOBAL YIELD IMPORT RETURN AS TYPE_COMMENT FOR 
%token DEL NONLOCAL ELIF NEWLINE TRUE NONE CLASS DEDENT FINALLY WITH EXPECT IS MATCH CONTINUE RAISE  
%token PASS LAMBDA DEF FALSE CASE ASSERT AWAIT BREAK ASYNC TRY 
%token NEWLINE INDENT DEDENT TYPE_COMMENT ASYNC AWAIT   YIELD LAMDA
%token '+' '-' '*' '/' '(' ')' '[' ']' ';' '.' '!' ':' '|' ',' '%' '^' '@' '{' '}' '=' 
%token '&' '~' '?' '_' NOT OR AND
%token ELIF THEN FOR DO IMPORT NONE TRUE FALSE IS AS DEF TRY EXCEPT FINALLY
